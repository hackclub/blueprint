...
#  demo_link                   :string
#  description                 :text
#  design_resubmission_count   :integer          default(0), not null
...
class Project < ApplicationRecord
  ...
  def generate_journal(include_time)
    include_time ||= false
    latest_entry = journal_entries.order(created_at: :desc).pick(:id, :updated_at)
    cache_key = [
      "project_journal",
      id,
      include_time,
      ...
      latest_entry&.first,
      latest_entry&.second&.to_i
    ].join("/")

    Rails.cache.fetch(cache_key, expires_in: 1.day) do
      build_journal_content(include_time)
    end
  end

  def build_journal_content(include_time)

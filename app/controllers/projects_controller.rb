class ProjectsController < ApplicationController
  ...
  def explore
    ...
    if params[:type] == "journals"
      ...
      if params[:sort] == "new"
        scope = Project.joins(:user).where(is_deleted: false).listed.with_attached_demo_picture.includes(:latest_journal_entry, :user).order(created_at: :desc)
        ...
        if followed_project_ids.empty?
          ...
          scope = Project.joins(:user).where(id: followed_project_ids, is_deleted: false).listed.with_attached_demo_picture.includes(:latest_journal_entry, :user).order(created_at: :desc)
          scope = scope.where("projects.title ILIKE :q OR projects.description ILIKE :q", q: "%#{search_query}%") if search_query.present?
          @pagy, @projects = pagy(scope, limit: 24)
          preload_project_metrics(@projects)
        end
        ...
        if current_user && Flipper.enabled?(:gorse_recommendations, current_user)
          page = params[:page].present? ? params[:page].to_i : 1
          project_ids = GorseService.get_user_recommendation(current_user.id, page, 24, type: :project)
          ...
          # Calculate count: if empty, we've reached the end
          count = project_ids.empty? ? (page - 1) * 24 : page * 24 + 1

          # Create manual pagy object for navigation
          @pagy = Pagy.new(count: count, page: page, items: 24, overflow: :last_page)
          ...
          if project_ids.any?
            ...
            scope = Project.where(id: project_ids).listed.with_attached_demo_picture.includes(:latest_journal_entry)
            scope = scope.where("projects.title ILIKE :q OR projects.description ILIKE :q", q: "%#{search_query}%") if search_query.present?
          ...
          scope = Project.where(id: paginated_ids).with_attached_demo_picture.includes(:latest_journal_entry)
          scope = scope.where("projects.title ILIKE :q OR projects.description ILIKE :q", q: "%#{search_query}%") if search_query.present?
          @projects = scope.order(Arel.sql(order_clause))
          preload_project_metrics(@projects)
        end
      elsif params[:sort] == "top"
        scope = Project.joins(:user).where(is_deleted: false).listed.with_attached_demo_picture.includes(:latest_journal_entry, :user).order(views_count: :desc)
        scope = scope.where("projects.title ILIKE :q OR projects.description ILIKE :q", q: "%#{search_query}%") if search_query.present?
        @pagy, @projects = pagy(scope, limit: 24)
        preload_project_metrics(@projects)
      elsif params[:sort] == "shipped"
        scope = Project.joins(:user).where(is_deleted: false, review_status: "build_approved").listed.with_attached_demo_picture.includes(:latest_journal_entry, :user).order(created_at: :desc)
        scope = scope.where("projects.title ILIKE :q OR projects.description ILIKE :q", q: "%#{search_query}%") if search_query.present?
        @pagy, @projects = pagy(scope, limit: 24)
        preload_project_metrics(@projects)
      else
